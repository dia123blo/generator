...
from utils_Patterns_exist_gen import generate_edge_label_by_device_types,generate_labels, generate_tree, get_direction, powerset, sample_element, str2bool, retrieve_multiple_edges,generate_vf_random_in_range_list
...
class GraphGenerator(object):
    def __init__(self, pattern):
        ...
        # 确保pattern的边标签是字符型
        self.pattern.vs["label"] = [int(x) for x in self.pattern.vs["label"]]
        self.pattern.es["label"] = [int(x) for x in self.pattern.es["label"]]
...
# ... existing code ...
    def _generate_fallback(self, number_of_vertices, number_of_edges, number_of_vertex_labels, number_of_edge_labels):
        ...
        for e, edge in enumerate(graph.es):
            # 根据源节点和目标节点的设备类型生成边标签
            source_label = vertex_labels[edge.source]
            target_label = vertex_labels[edge.target]
            edge_label = str(generate_edge_label_by_device_types(int(source_label), int(target_label)))
            graph_edge_label_mapping[(0, edge.source, 0, edge.target)].add(edge_label)
        ecount = graph.ecount()
        edge_keys = [0] * ecount
...
# ... existing code ...
    ...
    def generate(self, number_of_vertices, number_of_edges, number_of_vertex_labels, number_of_edge_labels,
        ...
        if not graph_pattern_valid:
            ...
            for sg in range(number_of_subgraphs):
                ...
                # 确保顶点标签和边标签为字符型
                subgraph_vertex_labels = [int(x) for x in generate_vf_random_in_range_list(number_of_subgraph_vertices)]
                # 使用基于设备类型的边标签生成而不是随机生成
                ...
                # 为子图中的边生成基于设备类型的标签
                subgraph_edge_labels = []
                if number_of_subgraph_vertices > 1:
                    # 先创建一个临时树结构来确定边的连接
                    temp_tree = generate_tree(number_of_subgraph_vertices, directed=True)
                    for edge in temp_tree.es:
                        source_label = subgraph_vertex_labels[edge.source]
                        target_label = subgraph_vertex_labels[edge.target]
                        edge_label = int(generate_edge_label_by_device_types(int(source_label), int(target_label)))
                        subgraph_edge_labels.append(edge_label)
            ...
            for sg, subgraph in enumerate(subgraphs):
                if subgraph.ecount() == 0 and numbers_of_subgraph_vertices[sg] > 1:
                    ...
                    # 生成边标签，使用generate_edge_label_by_device_types考虑器件类型
                    ...
                    edge_label = int(generate_edge_label_by_device_types(source_label, target_label))
    ...
    def add_edges(self, subgraph1, subgraph2, graph_edge_label_mapping, graph_number_of_edge_labels, number_of_edges):
        ...
        while invalid_cnt < 10 and new_ecount < number_of_edges:
            ...
            # 获取节点标签（器件编码），保持字符串类型
            source_label = str(subgraph1.vs[v1]["label"])
            target_label = str(subgraph2.vs[v2]["label"])
            ...
            try:
                ...
                if source_label.isdigit() and target_label.isdigit():
                    ...
                    # 创建反向边标签，通过查找weight_pairs中对应的反向权重
                    # edge_label格式: size_relation(1位) + out_to_in(3位)
                    if len(str(edge_label)) >= 4:
                        size_relation = str(edge_label)[0]
                        out_to_in = str(edge_label)[1:4]
                        
                        # 查找对应的权重对
                        reverse_out_to_in = out_to_in  # 默认值
                        for (forward, backward), count in weight_pairs:
                            if str(forward) == out_to_in:
                                reverse_out_to_in = backward
                                break
                            elif str(backward) == out_to_in:
                                reverse_out_to_in = forward
                                break
                        
                        # 根据大小关系确定反向边的大小关系
                        if size_relation == '9':  # 正向大
                            reversed_size_relation = '2'  # 反向大
                        elif size_relation == '2':  # 反向大
                            reversed_size_relation = '9'  # 正向大
                        else:  # '5' 相等或其他情况保持不变
                            reversed_size_relation = size_relation
                            
                        reverse_edge_label = reversed_size_relation + str(reverse_out_to_in).zfill(3)
                    else:
                        # 如果标签长度不正确，使用默认方式处理
                        reverse_edge_label = "0015"
                    
                    # 检查生成的边标签是否在pattern边标签中，如果在则跳过
                    if hasattr(self, 'pattern_edge_label_list') and edge_label in self.pattern_edge_label_list:
                        invalid_cnt += 1
                        continue
                    ...
                    if sg1 != sg2 or v1 != v2:
                        ...
                        # 检查反向边标签是否在pattern边标签中
                        if (hasattr(self, 'pattern_edge_label_list') and 
                            reverse_edge_label not in self.pattern_edge_label_list and
                            reverse_edge_label not in graph_edge_labels_reverse):
                            graph_edge_labels_reverse.add(reverse_edge_label)
                            new_ecount += 1
                    
                else:
                    # 如果标签不是数字，从pattern边标签中随机选择
                    edge_label = np.random.choice(self.pattern_edge_label_list) if hasattr(self, 'pattern_edge_label_list') and len(self.pattern_edge_label_list) > 0 else "0015"
                ...
                # 检查默认标签是否在pattern边标签中，如果在则重新生成一个不在其中的标签
                if hasattr(self, 'pattern_edge_label_list') and edge_label in self.pattern_edge_label_list:
                    # 生成一个不在pattern中的标签
                    candidate_labels = [lbl for lbl in ["0025", "0035", "0045"] if lbl not in self.pattern_edge_label_list]
                    if candidate_labels:
                        edge_label = candidate_labels[0]
                    else:
                        edge_label = "0025"  # fallback标签
