...
def generate_vf_random_in_range(min_val, max_val):
    ...
    """
    device_type_list = ['0', '1', '2', '3', '004', '005']
    intra_class_code_list = ['000', '001']
    ...
    return str(device_type)+str(intra_class_code)
...
def generate_edge_label_by_device_types(source_device, target_device, connection_type="default"):
    ...
    source_type = str(source_device)[:3]
    target_type = str(target_device)[:3]
    ...
    # 组合成32位编码
    edge_code = str(out_to_in).zfill(3) + str(in_to_out).zfill(3) + size_relation
    return str(edge_code)

# def generate_edge_label_by_device_types(source_device, target_device, connection_type="default"):
#     """
#     根据器件类型生成完整的边编码
#     完整格式：前3位（输出→输入）+ 中3位（输入→输出）+ 后2位（大小关系）
#     """
#     try:
#         # 确保输入是整数或可转换为整数的字符串
...
#         # 大小关系编码选项
#         size_relation_options = ['1', '5', '9']
#         size_relation = random.choice(size_relation_options)
        
#         # 有源器件：nmos(0), pmos(1), BJT(5)
#         active_devices = ['01','00', '11', '10']  # 字符串形式
#         # 无源器件：电容(2), 电阻(3), 二极管(4)
#         passive_devices = ['21','20', '30','31', '40', '50']
...
#         # 避免全0的编码
#         if edge_code == '00000000':
#             edge_code = '00100199'  # 默认非零编码
