...
from utils_P import generate_edge_label_by_device_types, generate_labels, generate_tree, get_direction, powerset, sample_element, str2bool, retrieve_multiple_edges, generate_vf_random_in_range_list, generate_ef_random_in_range_list, generate_ef_random_in_range
...
class GraphGenerator(object):
    ...
    def generate(self, number_of_vertices, number_of_edges, number_of_vertex_labels, number_of_edge_labels,
        ...
        if not graph_pattern_valid:
            ...
            for sg in range(number_of_subgraphs):
                ...
                subgraph_vertex_labels = generate_vf_random_in_range_list(number_of_subgraph_vertices)
                
                # 随机选择部分顶点设置为模板图的标签
                num_pattern_vertices = min(self.number_of_pattern_vertices, number_of_subgraph_vertices)
                pattern_indices = np.random.choice(number_of_subgraph_vertices, size=num_pattern_vertices, replace=False)
                for i, idx in enumerate(pattern_indices):
                    if i < len(self.pattern_vertex_label_list):
                        subgraph_vertex_labels[idx] = self.pattern_vertex_label_list[i]
                
                subgraph_edge_labels = generate_ef_random_in_range_list(number_of_subgraph_vertices-1)
            ...
            if return_subisomorphisms:
                ...
                scount = copy.deepcopy(len(subisomorphisms))
                ...
                metadata = {"counts": counts, "subisomorphisms": subisomorphisms}
    ...
    def _generate_fallback(self, number_of_vertices, number_of_edges, number_of_vertex_labels, number_of_edge_labels):
        ...
        subisomorphisms = self.pattern_checker.get_subisomorphisms(graph, self.pattern) if counts > 0 else []
    ...
    def add_edges(self, subgraph1, subgraph2, graph_edge_label_mapping, graph_number_of_edge_labels, number_of_edges):
        ...
        while invalid_cnt < 10 and new_ecount < number_of_edges:
            ...
            if get_direction():
                ...
                edge_label = generate_edge_label_by_device_types(target_label, source_label)
                y = (sg2, v2, sg1, v1)
        ...
        for i, subisomorphism in enumerate(subisomorphisms):
            for pattern_vertex, graph_vertex in enumerate(subisomorphism):
                ...
                for pattern_edge in pattern.incident(pattern_vertex):
                    ...
                    if (subisomorphism[pattern_tgt], edge_label) in graph_edge_dict:
...
if __name__ == "__main__":
    ...
    # 生成多个图
    results = []
    for g in range(args.number_of_graphs):
        graph, metadata = graph_generator.generate(
            args.number_of_vertices, args.number_of_edges,
            args.number_of_vertex_labels, args.number_of_edge_labels,
            args.alpha, max_pattern_counts=args.max_pattern_counts, 
            max_subgraph=args.max_subgraph,
            return_subisomorphisms=args.return_subisomorphisms
        )
        print("%d/%d" % (g+1, args.number_of_graphs), "number of subisomorphisms: %d" % (metadata["counts"]))
        results.append((graph, metadata))

    # 保存结果
    if args.save_graph_dir:
        os.makedirs(args.save_graph_dir, exist_ok=True)
        pattern_name = os.path.splitext(os.path.basename(args.pattern_path))[0]
        save_graph_dir_pattern = os.path.join(args.save_graph_dir, pattern_name)
        os.makedirs(save_graph_dir_pattern, exist_ok=True)
        
        if args.save_metadata_dir:
            os.makedirs(args.save_metadata_dir, exist_ok=True)
            save_metadata_dir_pattern = os.path.join(args.save_metadata_dir, pattern_name)
            os.makedirs(save_metadata_dir_pattern, exist_ok=True)
        
        for g, (graph, metadata) in enumerate(results):
            graph_id = "G_N%d_E%d_NL%d_EL%d_%d" % (
                graph.vcount(), graph.ecount(), 
                args.number_of_vertex_labels, args.number_of_edge_labels, g)
            
            # 保存图
            graph_filename = os.path.join(save_graph_dir_pattern, graph_id + ".gml")
            graph.write(graph_filename)
            
            # 保存元数据
            if args.save_metadata_dir:
                metadata_filename = os.path.join(save_metadata_dir_pattern, graph_id + ".meta")
                with open(metadata_filename, "w") as f:
                    json.dump(metadata, f)
            
            # 保存PNG图像
            if args.save_png:
                png_filename = os.path.join(save_graph_dir_pattern, graph_id + ".png")
                ig.plot(graph, png_filename)
            
            # 显示图像
            if args.show_img and metadata["subisomorphisms"]:
                graph_generator.draw(graph, pattern, metadata["subisomorphisms"])