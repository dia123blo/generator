...
from utils_l2f import generate_labels, generate_tree, get_direction, powerset, sample_element, str2bool, retrieve_multiple_edges,generate_vf_random_in_range_list,generate_ef_random_in_range_list,generate_ef_random_in_range
...
class GraphGenerator(object):
    ...
    def generate(self, number_of_vertices, number_of_edges, number_of_vertex_labels, number_of_edge_labels,
        ...
        if not graph_pattern_valid:
            ...
            #generate_vf_random_in_range_list,generate_ef_random_in_range_list
            ...
            if len(subisomorphisms) == 0:
                default_mapping = [999,999,999] 
                subisomorphisms.append(default_mapping)
            ...
            for sg in range(number_of_subgraphs):
                ...
                # 从pattern_vertex_label_list中随机选择若干元素替换subgraph_vertex_labels中的元素，保持长度不变
                ...
                # 从pattern_edge_label_list中随机选择若干元素替换subgraph_edge_labels中的元素，保持长度不变
            ...
            if return_subisomorphisms:
                subisomorphisms = dict()  # 使用字典存储不同模式的子图同构
                counts = dict()  # 存储不同模式的计数
                
                # 检测主模式
                for sg, subgraph in enumerate(subgraphs):
                    main_subisomorphisms = self.pattern_checker.get_subisomorphisms(subgraph, self.pattern)
                    main_subisomorphisms = [ [graph_vertex_mapping_reversed[(sg, v)] for v in subisomorphism] 
                                            for subisomorphism in main_subisomorphisms ]
                    if "main" not in subisomorphisms:
                        subisomorphisms["main"] = []
                    subisomorphisms["main"].extend(main_subisomorphisms)
                
                # 如果有其他子模式，也进行检测
                # 这里可以根据需要添加对其他子模式的检测逻辑
                
                counts["main"] = len(subisomorphisms["main"])
                ...
                if len(subisomorphisms) ==0:
                    default_mapping = [999] * int(self.pattern.vcount())
                    subisomorphisms.append(default_mapping)
                metadata = {"counts": counts, "subisomorphisms": subisomorphisms}
    ...
    def add_edges(self, subgraph1, subgraph2, graph_edge_label_mapping, graph_number_of_edge_labels, number_of_edges):
        ...
        while invalid_cnt < 10 and new_ecount < number_of_edges:
            ...
            edge_label = generate_ef_random_in_range(64, 319)
            if get_direction():
